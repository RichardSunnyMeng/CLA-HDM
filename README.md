...
## Inference
&ensp; &ensp; CLA-HDM determined the specific pathological type based on the diagnosis conclusion of benign and malignant. Specifically, let the probability of malignant by sub-model 1 be p~1~, the probability of tuberculous and reactive by sub-model 2 be p~2~(Tuberculous)and p~2~(Reactive), the probability of lymphoma and metastatic by sub-model 3 be p~3~(Lymphoma) and p~3~(Metastatic), respectively. Then the probability for each pathological type by CLA-HDM on each pathological type is:
